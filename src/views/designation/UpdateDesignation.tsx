...
const UpdateDesignation: React.FC = () => {
    ...
    const onUpdate = async (event: any) => {
        ...
        if (designationStore?.isValidCreateDesignationForm()) {
            isValidForm = true;
            await DesignationHelper(navigate).UpdateDesignation();
            ...
            <div className="row mb-2">
                <div className="col-4">
                    <div className="col" style={{ fontWeight: '600', fontSize: '18px' }}>
                        <span style={{ color: '#000000' }}>Update Designation</span>
                    </div>
                </div>
            </div>
            <div className="row">
                <div className="col-4">
                    <FormGroup isRequired label='Department' labelSpacing='mb-1' error={designationStore?.formCreateDesignationErrors?.departmentId}>
                        <Select placeholder="Select Department" className="custom-input" style={{ width: '100%' }}
                            onChange={(value) => onChangeSelectValue(value, 'departmentId')} defaultValue={designationStore.departmentId}>
                ...
                <div className="col-4">
                    <FormGroup isRequired label='Role' labelSpacing='mb-1' error={designationStore?.formCreateDesignationErrors?.roleId}>
                        <Select placeholder="Select Role" className="custom-input" style={{ width: '100%' }}
                            onChange={(value) => onChangeSelectValue(value, 'roleId')} defaultValue={designationStore.roleId}>
...
export default observer(UpdateDesignation);
